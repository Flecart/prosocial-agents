#!/usr/bin/env bash
# Run gpt-5.4 on NL contracting + i.i.d. stochastic regeneration (seeds 0-4).
# Usage: bash scripts/stochastic/run_iid_stochastic_nl.sh
# Override model: MODEL=openai/gpt-4o bash scripts/stochastic/run_iid_stochastic_nl.sh
set -euo pipefail

MODEL=${MODEL:-"openai/gpt-5.4-mini"}
SEEDS=${SEEDS:-"0 1 2 3 4"}
GROUP=${GROUP:-"iid_stochastic_nl-5.4-mini"}
CONTRACT_MODE=${CONTRACT_MODE:-"code_nl"}
...
for seed in $SEEDS; do
  ...
  uv run python -m simulation.main \
    experiment=fish_iid_stochastic_code_nl \
    group_name="$PATH_NAME" \
    seed="$seed" \
    llm.path="$MODEL" \
    "experiment.contracting.mode=$CONTRACT_MODE" \

#!/usr/bin/env bash
# Run no-contract baseline + i.i.d. stochastic regeneration (seeds 0-4).
# Usage: bash scripts/stochastic/run_iid_stochastic_no_contract.sh
# Override model: MODEL=openai/gpt-4o bash scripts/stochastic/run_iid_stochastic_no_contract.sh
set -euo pipefail

MODEL=${MODEL:-"openai/gpt-5.4-mini"}
SEEDS=${SEEDS:-"0 1 2 3 4"}
GROUP=${GROUP:-"no_contract-5.4-mini"}
...
for seed in $SEEDS; do
  ...
  uv run python -m simulation.main \
    experiment=fish_iid_stochastic_no_contract \
